...
describe('User API', () => {
    it('should respond 200 and code 0 and get user list', async () => {
        const res = await chai.request(app).get('/user/al');
    ...
    it('should respond 200 and code 0 and get borrowing list by userid', async () => {
        const res = await chai.request(app).get('/user/test');
        expect(res).to.have.status(200);
        expect(res.body).to.have.property('code');
        expect(res.body).to.have.property('code', 0);
